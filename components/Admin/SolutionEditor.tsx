...
import { Solution, Language, EnrichedSolution, DigitalAgent } from '../../types';
...
const SolutionEditor: React.FC<SolutionEditorProps> = ({ solution, language, onSave, onCancel }) => {
  ...
  const [activeTab, setActiveTab] = useState<'basic' | 'specs' | 'execution' | 'workforce'>('basic');
  const [availableAgents, setAvailableAgents] = useState<DigitalAgent[]>([]);
  const [isSelectingAgent, setIsSelectingAgent] = useState(false);

  useEffect(() => {
    api.agents.list().then(res => {
      if (res.success && res.data) setAvailableAgents(res.data);
    });
  }, []);

  const [formData, setFormData] = useState<Partial<Solution & { scenarios: string[], agents: any[] }>>({
    ...
    tags: solution?.tags || [],
    agents: (solution as any)?.digitalTeam || [],
      ...
      <div className="flex gap-8">
        ...
        <div className="w-64 space-y-1">
          {[
            ...
            { id: 'workforce', label: 'Digital Team', icon: Users },
        ...
        <div className="flex-1 space-y-8 bg-slate-800 border border-slate-700 rounded-3xl p-10">
          ...
          {activeTab === 'workforce' && (
            <div className="space-y-8">
              <div className="flex justify-between items-center">
                <h3 className="text-sm font-black text-slate-500 uppercase tracking-widest">Team Composition</h3>

                <div className="relative">
                  {!isSelectingAgent ? (
                    <button
                      onClick={() => setIsSelectingAgent(true)}
                      className="text-xs text-blue-400 flex items-center gap-1 hover:text-blue-300 font-bold bg-blue-500/10 px-3 py-1.5 rounded-lg transition-colors border border-blue-500/20"
                    >
                      <Plus size={14} /> Add Agent from Workshop
                    </button>
                  ) : (
                    <div className="flex items-center gap-2 animate-in fade-in zoom-in-50 duration-200">
                      <select
                        className="bg-slate-900 border border-blue-500 rounded-lg py-1.5 px-3 text-xs text-white outline-none w-56"
                        onChange={(e) => {
                        ...
                        autoFocus
                        onBlur={() => setTimeout(() => setIsSelectingAgent(false), 200)}
                      >
                      ...
                      <button onClick={() => setIsSelectingAgent(false)} className="p-1 text-slate-500 hover:text-white"><X size={14} /></button>
                    </div>
                  )}
                </div>
              </div>
              ...
              <div className="grid grid-cols-1 gap-4">
                {formData.agents?.map((agent, idx) => (
                  <div key={idx} className="bg-slate-900 border border-slate-700 rounded-xl p-4 flex gap-4 items-center group">
                    <img src={`https://picsum.photos/seed/${agent.avatarSeed}/100`} className="w-12 h-12 rounded-lg bg-slate-800" />
                    <div className="flex-1 grid grid-cols-2 gap-4">
                      <div className="space-y-1">
                        <label className="text-[9px] text-slate-500 uppercase font-black">Name</label>
                        <input
                          value={agent.name}
                          onChange={e => {
                            const next = [...(formData.agents || [])];
                            next[idx] = { ...next[idx], name: e.target.value };
                            setFormData({ ...formData, agents: next });
                          }}
                          className="w-full bg-slate-900 border border-slate-800 rounded px-2 py-1 text-xs text-white focus:border-blue-500 outline-none"
                        />
                      </div>
                      <div className="space-y-1">
                        <label className="text-[9px] text-slate-500 uppercase font-black">Role</label>
                        <input
                          value={agent.role}
                          onChange={e => {
                            const next = [...(formData.agents || [])];
                    ...
                    <button
                      onClick={() => setFormData(prev => ({ ...prev, agents: prev.agents?.filter((_, i) => i !== idx) }))}
